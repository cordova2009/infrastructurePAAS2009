...
参数|是否必须|说明
----|----|-----
BeeBankInfo|是|招标人银行信息
BerBankInfo|是|投标人银行信息
bankId|是|银行账户信息内部记录编号
bank|是|银行开户行
accountId|是|银行账号
accountName|是|开户人
...
----|----|-----
time|是|交易时间
type|是|交易类型,SBZ收退还保证金，JBZ交纳保证金，GFK工程付款（招标方至平台），GSK工程收款（平台至投标方），CHZ冲正,TX#提现,CZ#充值
income|是|收入，单位元
outlay|是|支出，单位元
balance|是|结余，单位元
remark|是|备注
...
http请求方式: post
    ...
    http://IP:PORT/if/capitalManage/queryMyCapitalSurvey
