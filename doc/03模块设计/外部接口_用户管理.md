...
根据手机号码查询用户是否存在，用于登录，忘记密码等功能

1）请求说明

http请求方式: post

    http://IP:PORT/if/general/isUser
...
正常时的返回JSON数据包示例：
    ...
    {
        "errcode":0,"errmsg":"查询用户成功","status":"OK#"
    }

错误时的JSON数据包示例：

    {
        "errcode":10000,"errmsg":"查询用户失败，其它错误"
    }


参数|必须|说明
----|----|----
status|是|该用户存在状态，OK#普通存在用户，DIS，禁用用户，BLI，黑名单用户，NEX不存在的用户
...
http请求方式: post
    ...
    http://IP:PORT/if/userInfo/verifyIdCard
...
错误时的JSON数据包示例：
    ...
    {
        "errcode":10000,"errmsg":"校验身份证失败，其它错误"
    }


参数|必须|说明
----|----|----
isIdCard|是|是否验证通过,YES,通过,FLS ,不通过
