...
class medoo {
     ...
     * 返回数据库实例
     */
    public static function getMasterInstance($options=null) {
        static $master_instance;
        //如果缓存中没有，则重新构造
        if (!isset($master_instance)) {
            $classname = __CLASS__;
            $master_instance = new $classname($options);
        }
        //返回结果
    ...
    public function __construct($options = null) {
        try {
            ...
            switch ($type) {
                ...
                case 'mysql':
                    if ($this->socket) {
                        $this->dsn = $type . ':unix_socket=' . $this->socket . ';dbname=' . $this->database_name;
                    } else {
                        $this->dsn = $type . ':host=' . $this->server . ($is_port ? ';port=' . $port : '') . ';dbname=' . $this->database_name;
                    }
                ...
                case 'sybase':
                    $this->dsn = 'dblib:host=' . $this->server . ($is_port ? ':' . $port : '') . ';dbname=' . $this->database_name;
                    break;
                ...
                case 'oracle':
                    ...
                    $this->dsn = 'oci:dbname=' . $dbname . ($this->charset ? ';charset=' . $this->charset : '');
                    break;

                case 'mssql':
                    $this->dsn = strstr(PHP_OS, 'WIN') ?
                ...
                case 'sqlite':
                    $this->dsn = $type . ':' . $this->database_file;
                    $this->username = null;
                    $this->password = null;
                    break;
            }
    ...
    public function connect($retry = 3) {

        try {
            $this->pdo = new PDO($this->dsn, $this->username, $this->password, $this->option);
            //$this->pdo = new pdo_connect_pool($this->dsn, $this->username, $this->password, $this->option);
            foreach ($this->commands as $value) {
                $this->pdo->exec($value);
            }
        } catch (PDOException $e) {
            if ($retry == 0) {
                SeasLog::error('数据库查询重连3次失败 ERR_MSG'.$e->getMessage());
                throw new Exception("数据库重连3次失败！" . $e->getMessage(), $e->getCode());
            }
            $this->connect( --$retry);
        }
    }
    ...
    public function query($query, $retry = 3) {
        ...
        try {
            ...
            if ($retry == 0) {
                SeasLog::error('数据库查询重连3次失败SQL:'.$query.' ERR_MSG'.$e->getMessage());
                throw new Exception("数据库查询重连3次失败！" . $e->getMessage(), $e->getCode());
            }
            $this->connect();
            $result = $this->query($query, --$retry);
        }
        return $result;
    }

    public function exec($query, $retry = 3) {
        ...
        try {
            ...
            if ($retry == 0) {
                SeasLog::error('数据库查询重连3次失败SQL:'.$query.' ERR_MSG'.$e->getMessage());
                throw new Exception("数据库执行重连3次失败！" . $e->getMessage(), $e->getCode());
            }
            $this->connect();
            $result = $this->exec($query, --$retry);
        }
        return $result;
    }
                ...
                if (is_array($LIKE)) {
                    ...
                    foreach ($LIKE as $column => $keyword) {
                        ...
                        foreach ($keyword as $key) {
                            ...
                            $clause_wrap[] = $this->column_quote($column_match[2]) .
...
//                             $relation = 'ON ' . $table . '."' . key($relation) . '" = "' . (isset($match[5]) ? $match[5] : $match[3]) . '"."' . current($relation) . '"';
    ...
    public function insert($table, $datas,$hasPk=false) {
        $lastId = array();
        ...
        foreach ($datas as $data) {
            ...
            $flag = $this->exec('INSERT INTO "' . $table . '" (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')');
            if($hasPk){
                $lastId[] = $flag;
            }else{
    ...
    public function get($table, $join = null, $column = null, $where = null) {
        ...
        if($query){
            ...
            SeasLog::warning('没有找到结果SQl:'.$this->last_query());
        }
