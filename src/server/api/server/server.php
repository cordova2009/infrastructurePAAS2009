...
define ('CONF_PATH', '/var/www/html/conf/piao/');
...
class HttpServer
{
	...
	public function __construct() {
        ...
        self::$http = new swoole_http_server("0.0.0.0", $config->mallapi->port);
	...
	self::$http->on('request', function($request, $response) {
        ...
        self::$http->on('Task', function($serv, $task_id, $from_id, $data) {
            ...
            $this->my_log("新的异步任务[来自进程 {$from_id}，当前进程 {$task_id}],data:".$data);
	...
	public function onWorkerStart($serv, $worker_id) {
	    ...
	    $this->application = new Yaf\Application (CONF_PATH. 'app.ini', 'development');
