...
//                 var_dump($rule);die;
    ...
    final public function getNav($pid=null){
        ...
        if(empty($pid)){
            ...
            $nav = M('Menu')->where("url like '%".CONTROLLER_NAME."/".ACTION_NAME."%'")->field('id,pid,title')->find();
            ...
            $nav =  M('Menu')->field('id,pid,title')->find($pid);
            if($nav && $nav['pid']){
                $this->getNav($nav['pid']);
            }
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        ...
        if(!empty($menus)){
            ...
            foreach ($menus['main'] as $key => &$item) {
                ...
                if($item['id'] == $current['id']){
                    $class = empty($item['child']) ? 'active' : 'active open';
                    $item['class'] = $class;
                }else{
                    $item['class'] = '';
                }
        ...
        $menus['main']  =   M('Menu')->where($where)->order('sort asc')->field('id,title,url,class_fix')->select();
