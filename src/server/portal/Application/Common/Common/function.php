...
function show_thumb($url,$thumb){
	
    $info = pathinfo($url);
    
    return $info['dirname'].'/'.$thumb.'/'.$info['basename'];
}
 ...
 * @param  string $order 排序显示的键，一般是主键 升序排列，如果传空，则不排序
 * @param  int    $level  级别，会为每个item增加一个属性，如果已有此属性，则会覆盖，
 * @param  array  $list  过渡用的中间数组，
 * @return array        返回排过序的列表数组
 * @author yangweijie <yangweijiester@gmail.com> |　Modify: 王雪兵 <406964108@qq.com>
 */
function tree_to_list($tree, $child = '_child', $order='id', $level=1, &$list = array()){
    if(is_array($tree)) {
        foreach ($tree as $key => $value) {
            $reffer = $value;
            $reffer['level'] = $level;
            if(array_key_exists($child,$reffer)){
                unset($reffer[$child]);
            	$list[] = $reffer;
                tree_to_list($value[$child], $child, $order, $level+1, $list);
            }else{
 ...
 * @author 麦当苗儿 <zuojiazi@vip.qq.com>
 */
function addons_url($url, $param = array(),$module=''){
    $url        = parse_url($url);
