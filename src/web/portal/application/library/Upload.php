...
class Upload {
    ...
    /**
     * 上传文件
     * @param string $files 文件信息数组 $files ，通常是 $_FILES数组
     * @return array|bool
     */
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            if( $this->callback && $data ){
                if ( file_exists(dirname(dirname($this->config['rootPath'])).$data['path'])  ) {
                    $info[$key] = $data;
                    continue;
    ...
    private function setDriver($driver = null, $config = null){
        ...
        if(!$this->uploader){
            throw new Exception("不存在的上传驱动：{$driver}", 5);
        }
    }
